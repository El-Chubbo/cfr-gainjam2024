...
#To do: Focus on Cirana hotkey, focus on specific tile hotkey
...
var quick_target: Tween ##new variable, used for quickly panning the camera during specific events
var is_quick_panning = false
signal quick_pan_completed
...
func _on_quick_pan_event(new_position: Vector2):
	...
	quick_target.tween_property(self, "offset", new_position, 0.5)
	#quick_target.tween_property(self, "offset.y", new_position.y, 0.5)
	is_quick_panning = false
	quick_pan_completed.emit()
	return

func _on_quick_follow_event(target: Node2D, duration: float = 0.25):
	print_debug("Following entity ", target)
	if quick_target:
		quick_target.kill()
	#to do:follow target for a specified amount of time
	return
